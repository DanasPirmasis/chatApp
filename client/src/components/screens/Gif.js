...
function Gif({
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			const results = await axios(
				`https://api.giphy.com/v1/gifs/search?q=${'study'}&api_key=${API_KEY}&limit=10&offset=5`
			);
	...
	const handleGif = (el) => {
		setMessageState({
			...
			message: el.images.fixed_height.url,
		});
	...
	const handleSubmit = async (event) => {
		...
		let API_KEY = 'IM6DimkCTEDPnuVZQ9gXJe97FJutUlIM';
		const results = await axios(
			`https://api.giphy.com/v1/gifs/search?q=${search}&api_key=${API_KEY}&limit=10&offset=5`
		);
			...
			<div className="search_gif">
				<form>
					<input
						...
						type="text"
					></input>
					<button type="submit" className="gif__btn" onClick={handleSubmit}>
						Search
					</button>
				</form>
			</div>
